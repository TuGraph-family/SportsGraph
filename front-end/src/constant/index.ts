export const DEFAULT_FLAG =
  "https://mdn.alipayobjects.com/huamei_92awrc/afts/img/A*vtoaTbhGjEEAAAAAAAAAAAAADsvfAQ/original";

export const STRENGTH_TEXT_SRC = {
  WIN_ACTIVE:
    "https://mdn.alipayobjects.com/huamei_92awrc/afts/img/A*zlY3SZe9z0YAAAAAAAAAAAAADsvfAQ/original",
  WIN_INACTIVE:
    "https://mdn.alipayobjects.com/huamei_92awrc/afts/img/A*kbVpSaljgEcAAAAAAAAAAAAADsvfAQ/original",
  LOSE_ACTIVE:
    "https://mdn.alipayobjects.com/huamei_92awrc/afts/img/A*5JBnTp0LvAQAAAAAAAAAAAAADsvfAQ/original",
  LOSE_INACTIVE:
    "https://mdn.alipayobjects.com/huamei_92awrc/afts/img/A*WLiJQJGwQW8AAAAAAAAAAAAADsvfAQ/original",
};
